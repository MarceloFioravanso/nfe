# -*- coding: utf-8 -*-
"""
Teste para verificar se as colunas X, Y, Z e AL existem na planilha Excel
e se contêm os dados esperados de vencimento e parcela.
"""
import os
import pandas as pd
import logging
...
logger = logging.getLogger('teste_colunas_excel')
...
def verificar_colunas_excel():
    """
    Verifica se as colunas X, Y, Z e AL existem na planilha Excel.
    """
    ...
    try:
        ...
        logger.info(f"Colunas disponíveis: {list(df.columns)}")
          # Verifica se as colunas de vencimento e parcela estão presentes
        ...
        if colunas_ausentes:
            ...
            # Para vencimento (colunas X, Y, Z)
            for nome_alternativo in ['Dia Venc', 'Dia', 'Vencimento Dia', 'Dia Vencimento']:
                if nome_alternativo in df.columns:
                    logger.info(f"Possível alternativa para coluna X: '{nome_alternativo}'")
            
            for nome_alternativo in ['Mês Venc', 'Mês', 'Vencimento Mês', 'Mês Vencimento']:
                if nome_alternativo in df.columns:
                    logger.info(f"Possível alternativa para coluna Y: '{nome_alternativo}'")
                    
            for nome_alternativo in ['Ano Venc', 'Ano', 'Vencimento Ano', 'Ano Vencimento']:
                if nome_alternativo in df.columns:
                    logger.info(f"Possível alternativa para coluna Z: '{nome_alternativo}'")
                    
            # Para parcela (coluna AL)
            for nome_alternativo in ['Parcela', 'Nº Parcela', 'Número Parcela', 'Parcela Nº']:
                if nome_alternativo in df.columns:
                    logger.info(f"Possível alternativa para coluna AL: '{nome_alternativo}'")
...
def verificar_dados_exemplo():
    ...
    try:
        ...
        for i in range(min(3, len(df))):
            ...
            dia_venc = linha.get('Dia Venc.', 'N/A')
            mes_venc = linha.get('Mês Venc.', 'N/A')
            ano_venc = linha.get('Ano Venc.', 'N/A')
            parcela = linha.get('Parcela', 'N/A')
            ...
            if pd.notna(dia_venc) and pd.notna(mes_venc) and pd.notna(ano_venc):
                try:
